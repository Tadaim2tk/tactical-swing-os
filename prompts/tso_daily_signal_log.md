...
- **市場データが検証不能な日でも、レポートとTSO_LOGを省略しない**。検証不能な資産は
  rank=NO_TRADE / type=DATA_UNAVAILABLE / verified_status=unverified として記帳し、
  価格数値は空欄にする（推測で埋めない）。全資産の行を必ず出す。欠測日はサンプル欠落として
  較正を損なう（2026-07-22追加）
