...
- 本文に、**前日の方向あり判断それぞれについて invalidation が発動したか**を1行で記す
  （書式: `invalidation_check: <signal_id>=fired|not_fired|unknown, ...`）。
  invalidation は毎日191/193行に書かれているのに、**発動したかは一度も記録されていない**。
  実際の手仕舞いは「SLやTPまで引っ張らずシナリオ崩壊で降りる」であり、その基準が
  当たっていたかを測れる列が無かった（人間の運用の言明、2026-09-07）。
