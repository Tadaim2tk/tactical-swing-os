...
- **win_prob・expected_r は防御的に丸めない。真の主観確率を書く**。過小申告も過大申告も
  同じ較正誤差として月次採点される（2026-07-15追加、経緯は docs/gpt_prompt_changelog.md）
- 遡及採点で実証済みの失敗モードを繰り返さない: (1)供給ショック直後のmomentum追随、
  (2)entry帯未到達での成行追い、(3)資産の参照系列の取り違え（単位契約の表を参照）
- **SLはentry帯から、これまでの想定の約2倍の距離に置く**。リスク量はrisk_pct基準のまま
  （SLを広げた分ロットを小さくする）。狭いSLは日足ノイズに55%の確率で刈られていたことが
  執行シミュレーションで判明した（2026-07-17追加・8/1の月次較正で効果判定）
- **決済は5営業日の時間決済を基本方針とする**。TP1/TP2は参考目標として従来どおり記帳する。
  早期利確が、5日かけて出る方向エッジを削っていたため（2026-07-17追加・同上）
- **Web検索・市場データ取得がこの実行環境で使えない日は、タスク失敗ではない。
  「実行できません」という応答は禁止**。その日は検証作業を一切行わず、全10資産を
  rank=NO_TRADE / type=DATA_UNAVAILABLE / side=NONE / 価格列すべて空欄 /
  regime=UNKNOWN / verified_status=unverified で記帳した TSO_LOG を必ず出力する。
  この形式は価格数値を一切含まないため Web検索ゼロで完全に実行可能で、
  「推測を書かない」ルールと矛盾しない。**「一次情報で確認する」義務は価格数値を
  書く場合にのみ適用される**。欠測日はサンプル欠落として較正を損なう
  （2026-07-22追加、2026-07-23強化: データ不能日はこの形式の出力こそが正しい実行）
- **過去の実行ターンの結果に関わらず、毎回の実行でまずWeb検索を新規に試みる**。前回までの
  「検索できない」「データ取得不能」という結論を当日の前提として引き継がない
  （毎日が独立の試行。2026-07-23追加、駆動会話のリセットと同時適用）
