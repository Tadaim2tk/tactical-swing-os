...
> **2026-09-07 注記**: 本文書の「再設計しない」「統計手法は既存のものだけ」
> 「チェックリストをそのまま実行」は **Phase 29 限定**の指示である
> （governance_reform_2026-07 §3b）。再現可能な欠陥が示された場合は変更提案してよい。
> 条件は「欠陥の再現手順が先にあること」——都合のよい結果を得るための閾値変更とは区別する。
