...
## 既知の欠落と2026-08-26の是正（隠さない記録）

- **MCTX-20260825T144243Z は復元不能**。#117 のブランチ作業中に台帳をローカル再生成して
  コミットした際に置換され（→ MCTX-20260825T145630Z）、元の行の値はどのコミットにも
  残っていない。CI の追記経路（export_market_context_snapshot.append_snapshot）は削除を
  行わない設計で、事故原因ではない（原因はブランチでの手動再生成）。
- 2026-08-25 の初期2行に close_btc / chg_pct_btc 等の小文字列と close_BTC 等の大文字列が
  併存する列和集合が発生（新しい行は大文字だけ・旧い行は小文字だけが埋まる自己増殖構造、
  契約外18列）。**真の発生機構は読込側**: 旧 load_existing が calibration_io.read_csv
  （normalize_headers が全ヘッダを小文字化）経由で台帳を読むため、既存行の契約列が毎回
  小文字化され、追記時の concat で大小両ケースが併存した（2026-08-26 Codex事後レビューP1で
  特定・load_existing をケース保存読込へ修正）。2026-08-26 に**値の移送による一回限りの移行コミット**で契約43列へ正規化
  （再生成ではない・18セル移送・値衝突ゼロ）。同時に append_snapshot へ schema enforcement
  （契約外列があれば追記を例外で拒否）と、コミット済み台帳への不変条件テスト
  （src/test_ledger_invariants.py: ヘッダ完全一致・HEAD~比で snapshot_id 消失ゼロ・
  既存行の値不変）を導入。行の append-only と列の schema enforcement は別の不変条件として扱う。
