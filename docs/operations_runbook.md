...
### 最終日の発動が1行欄から落ちていた（2026-09-21 判明）

GPTは各判断を5営業日目の朝で1行欄から外していたが、**5本目の値動きを観測できるのは
その翌朝の確認が最初で最後**。本文の検証節には最終判定が書かれているのに台帳へ届かず、
`20260909_USDJPY_SELL`（9/16 に 156.31 まで上昇）と `20260912_BTC_SELL`（9/18 に 80850 を
上抜き）の fired が落ちていた。本文の明言を転記して回収し、fired は累計5件。
**5件中2件が最終日の発動**なので、放置すれば発動率は4割低く出ていたことになる。

突合の窓は「5本目の翌日の確認まで開ける」。警告が出たら、まずその日の本文の
「前回判断の検証」に signal_id 名指しの最終判定が無いか探す。あればそれを転記する
（GPT自身の明言であって推測ではない）。無ければ警告をそのまま残す。
...
そのまま発動日（±1日）になる。10月の較正で使えるのは、日次で聞き始めた
2026-09-09 以降に発動したぶんだけである。
