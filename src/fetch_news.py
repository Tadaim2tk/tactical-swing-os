...
def parse_feed(source: NewsSource, fetched_at: datetime, hours: int) -> list[dict]:
    try:
...
def write_outputs(rows: list[dict], fetched_at: datetime) -> None:
    ...
    json_path.write_text(json.dumps(df.to_dict(orient="records"), ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    lines = [
...
def fetch_news(hours: int = 48, max_items: int = 100) -> pd.DataFrame:
    fetched_at = now_utc()
    rows = []
    for source in load_sources():
        rows.extend(parse_feed(source, fetched_at, hours))
    rows = dedupe_rows(rows, max_items)
    write_outputs(rows, fetched_at)
...
def main() -> int:
    ...
    fetch_news(hours=max(1, args.hours), max_items=max(1, args.max_items))
    return 0
