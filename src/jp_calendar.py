...
── メンテナンス責任（重要） ──────────────────────────────────────────
このモジュールは祝日法の変更や臨時休場を自動取得しない。
以下の変更が生じた場合は手動で更新が必要:
  - 祝日の新設・廃止・日付変更（例: 2016年 山の日追加）
  - 天皇即位・国民的行事による臨時休場
  - 株式市場の年末年始スケジュール変更

検証対象として扱うこと: 祝日計算結果は定期的に東証の公式カレンダーと
照合し、差異があれば _public_holidays() を修正すること。
更新責任者: プロジェクトの年次メンテナンス担当（ヒューマンレビュー必須）。
"""
